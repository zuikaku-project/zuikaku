...
export default class ZuikakuErrorEvent extends ZuikakuListener {
    public async execute(ctx: CommandContext, error: Error, command?: ICommandComponent): Promise<void> {
        await ctx.send({
            embeds: [
        ...
        const embedLog = createEmbed("error")
            .setTitle(`OOF! An error has occurred! ${command ? `| ${command.meta.name} command` : ""} `);
