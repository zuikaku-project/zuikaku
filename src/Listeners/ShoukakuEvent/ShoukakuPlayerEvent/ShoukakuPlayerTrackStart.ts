            ...
            if (getGuildDatabase?.guildPlayer?.channelId) {
                ...
                const playerButton = new MessageActionRow().addComponents(
                    ...
                    new MessageButton()
                        ...
                        .setStyle(
                            dispatcher.queue.previous ? "PRIMARY" : "SECONDARY"
                        )
                        .setCustomId(
                            Utils.encodeDecodeBase64String("Player_LAST-TRACK")
                        )
                    ...
                    new MessageButton()
                        ...
                        .setEmoji(
                            dispatcher.queueRepeat
                                ? ":repeat_one:924117419960725534"
                                : dispatcher.trackRepeat
                                ? ":no_repeat:941342881845768282"
                                : ":repeat:891234382097031168"
                        )
                        .setStyle("PRIMARY")
                        .setCustomId(
                ...
                const lastMusicMessageId = await dispatcher.getText
                    ?.send({
                        components: [playerButton],
                        embeds: [
                            createEmbed("info")
                                .setAuthor({
                                    name: `${dispatcher.queue.current!.info
                                        .title!} (${dispatcher.queue.current!
                                        .durationFormated!})`
                                })
                                .setThumbnail(
