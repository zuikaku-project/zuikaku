...
export default class MALMangaCommand extends ZuikakuCommand {
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const { data: getManga } = await this.client.apis.weebs.jikan.getManga(
    ...
    private async sendMessage(
        ctx: CommandContext,
        final: JikanMangaInterface["data"][0]
    ): Promise<void> {
        const firstEmbed = new MessageEmbed()
            ...
            .setAuthor({
                name: "MyAnimeList",
                iconURL:
                    "https://images-ext-1.discordapp.net/external/9b2osjuxc7HKMDXbNHLEXlT-wTxnKUy4MBapcLnIV4s/https/i.ibb.co/D9g5b4s/myanimelist.png",
            ...
            .setDescription(
                [
                    final.title,
                    final.title_english,
                    final.title_japanese,
                    ...final.title_synonyms
                ]
            ...
            .addFields([
                ...
                },
                {
                    name: "Published",
                    value:
                        final.published.from && final.published.to
                            ? `<t:${(
                                  new Date(final.published.from).getTime() /
                                  1000
                              ).toFixed(0)}:D> ` +
                              "to" +
                              ` <t:${(
                                  new Date(final.published.to).getTime() / 1000
                              ).toFixed(0)}:D>`
    ...
    private async searchFlag(
        ctx: CommandContext,
        JikanMedia: JikanMangaInterface["data"]
    ): Promise<void> {
        const mangaSlice = JikanMedia.map((x, i) => ({
            label: `${
                x.title.length > 90 ? `${x.title.substring(0, 90)}...` : x.title
            } [⭐${x.scored}]`,
            value: `${i++} `
        }));
        const rowSelectMenu = new MessageActionRow().addComponents(
            new MessageSelectMenu()
                .setCustomId(Utils.encodeDecodeBase64String("Jikan-Manga"))
            ...
            if (int.isSelectMenu()) {
                ...
                if (int.user.id === ctx.author.id) {
                    ...
                    await this.sendMessage(
                        ctx,
                        JikanMedia[Number(int.values[0])]
                    );
                } else {
                    await int.reply({
                        embeds: [
