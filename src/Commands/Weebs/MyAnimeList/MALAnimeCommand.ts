...
export default class MALAnimeCommand extends ZuikakuCommand {
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const { data: getAnime } = await this.client.apis.weebs.jikan.getAnime(
    ...
    private async sendMessage(
        ctx: CommandContext,
        final: JikanAnimeInterface["data"][0]
    ): Promise<void> {
        const firstEmbed = new MessageEmbed()
            ...
            .addFields([
                ...
                },
                {
                    name: "Aired",
                    value:
                        final.aired.from && final.aired.to
                            ? `<t:${(
                                  new Date(final.aired.from).getTime() / 1000
                              ).toFixed(0)}:D> ` +
                              "to" +
                              ` <t:${(
                                  new Date(final.aired.to).getTime() / 1000
                              ).toFixed(0)}:D>`
                            : final.aired.from
                            ? `<t:${(
                                  new Date(final.aired.from).getTime() / 1000
    ...
    private async searchFlag(
        ctx: CommandContext,
        JikanMedia: JikanAnimeInterface["data"]
    ): Promise<void> {
        ...
        const rowSelectMenu = new MessageActionRow().addComponents(
            new MessageSelectMenu()
                .setCustomId(Utils.encodeDecodeBase64String("Jikan-Anime"))
