...
export default class StopCommand extends ZuikakuCommand {
    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        queue.destroyPlayer();
        await ctx.send({ embeds: [createMusicEmbed(ctx, "info", "You has been stopped Music Player")] }).then(x => fromGuildPlayer ? setTimeout(() => x.delete().catch(() => null), 5000) : undefined).catch(() => null);
    }
}
