...
export default class TreblebassCommand extends ZuikakuCommand {
    @isMusicPlaying()
    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        if (queue.audioFilters.has("treblebass")) {
            queue.audioFilters.setTreblebass(false);
        } else {
            queue.audioFilters.setTreblebass();
        }
        await ctx.send({ embeds: [createEmbed("info", `**Treblebass filter has been ${queue.audioFilters.has("treblebass") ? "activated" : "deactivated"}**`)] }).then(x => {
            if (fromGuildPlayer) {
