...
export default class TreblebassCommand extends ZuikakuCommand {
    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        if (dispatcher.filter.has("treblebass")) {
            dispatcher.filter.setTreblebass(false);
        } else {
            dispatcher.filter.setTreblebass();
        }
        await ctx.send({ embeds: [createEmbed("info", `**Treblebass filter has been ${dispatcher.filter.has("treblebass") ? "activated" : "deactivated"}**`)] }).then(x => {
            if (fromGuildPlayer) {
