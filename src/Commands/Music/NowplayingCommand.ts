...
export default class NowplayingCommand extends ZuikakuCommand {
    ...
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const math =
            (timesCurrentDuration -
                (timesCurrentDuration - dispatcher.player.position)) /
            1000;
        const final = Utils.progressBar(
            dispatcher.queue.current?.info.isStream
                ? 1
                : dispatcher.queue.current!.info.length! / 1000,
            dispatcher.queue.current?.info.isStream ? 1 : math,
            15
        );
        ...
        const time = dispatcher.queue.current?.info.isStream
            ? "◉ LIVE"
            : `${
                  Utils.parseMs(progress, { colonNotation: true }).colonNotation
              }/${dispatcher.queue.current!.durationFormated!}`;
        const duration = createEmbed("info")
            ...
            .setDescription(
                `${
                    ...
                    `Requested by \`【${
                        dispatcher.queue.current!.requester!.username
                    }】\`**`
            )
            .setImage(dispatcher.queue.current!.thumbnail!);
        const generateLastNowplayingMessage = await ctx
            .send({
                embeds: [duration],
                deleteButton: {
            ...
            .catch(() => null);
        dispatcher.queueMessage.lastNowplayingMessage =
            generateLastNowplayingMessage ?? null;
    }
}
