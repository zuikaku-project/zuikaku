...
export default class BanCommand extends ZuikakuCommand {
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const getUser = await this.client.users
                ...
                await ctx
                    .send({
                        embeds: [
                            createEmbed(
                                "error",
                                "**<a:decline:879311910045097984> | I can't ban this user**"
                            )
                        ]
                    })
                    .then(x =>
                ...
                await ctx
                    .send({
                        embeds: [
                            createEmbed(
                                "error",
                                "**<a:decline:879311910045097984> | You can't ban yourself**"
                            )
                        ]
                    })
                    .then(x =>
                ...
                await ctx
                    .send({
                        embeds: [
                            createEmbed(
                                "error",
                                "**<a:decline:879311910045097984> | I can't ban myself**"
                            )
                        ]
                    })
                    .then(x =>
        ...
        if (!databan) {
            await ctx.send({
                embeds: [
                    createEmbed(
                        "info",
                        "**<a:decline:879311910045097984> | Sorry, I can't fetch guild band**"
                    )
                ]
            });
            return undefined;
            ...
            await ctx
                .send({
                    embeds: [
                        createEmbed(
                            "error",
                            "**<a:decline:879311910045097984> | This user has been banned from this guild**"
                        )
                    ]
                })
                .then(x =>
        ...
        try {
            await ctx.guild?.bans.create(getUser, {
            ...
            await ctx
                .send({
                    embeds: [
                        createEmbed(
                            "success",
                            `**<a:accept:884700222951931964> | Operation to ban **\`${tag}\`** successful!**`
                        )
                    ]
                })
                .then(x =>
            ...
            await ctx
                .send({
                    embeds: [
                        createEmbed(
                            "error",
                            `**Sorry i couldn't ban this user because \`\`\`js\n${e.message}\n\`\`\`**`
                        )
                    ]
                })
                .then(x =>
