...
export default class ServerinfoCommand extends ZuikakuCommand {
    public async execute(ctx: CommandContext): Promise<void> {
        ...
        const sver = await this.client.utils.firstUppercase(ctx.guild!.verificationLevel);
        ...
        const e = createEmbed("info")
            ...
            .addFields([
                ...
                { name: "Boost Level", value: sbos || !sbos ? "Not Boosted" : `Level ${sbos}` }
            ])
            .setThumbnail(ctx.guild!.iconURL({ dynamic: true, size: 4096, format: "png" })!)
