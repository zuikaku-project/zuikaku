...
export class JikanManager {
    ...
    public async getCharacters(
        querySearch: string
    ): Promise<JikanCharactersInterface> {
        return this.fetchJikan<JikanCharactersInterface>(
            querySearch,
            "characters"
        );
    }

    private async fetchJikan<JikanAnimeInterface>(
        querySearch: string,
        type: "anime"
    ): Promise<JikanAnimeInterface>;
    private async fetchJikan<JikanMangaInterface>(
        querySearch: string,
        type: "manga"
    ): Promise<JikanMangaInterface>;
    private async fetchJikan<JikanCharactersInterface>(
        querySearch: string,
        type: "characters"
    ): Promise<JikanCharactersInterface>;
    private async fetchJikan<T>(
        querySearch: string,
        type: "anime" | "characters" | "manga"
    ): Promise<T> {
