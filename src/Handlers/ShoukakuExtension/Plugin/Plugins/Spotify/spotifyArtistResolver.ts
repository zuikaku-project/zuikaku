...
import { IPluginComponent, LavalinkTrack, SpotifyArtist } from "@zuikaku/types";
    ...
    public async fetch(trackId: string): Promise<TrackList | undefined> {
        try {
            ...
            const spotifyArtis: SpotifyArtist = await petitio(`${this.plugin.spotifyBaseURL}/artists/${trackId}/top-tracks`)
                ...
                .header("Authorization", this.plugin.spotifyToken)
                .json();
            const unresolvedSpotifyTracks = spotifyArtis.tracks.map(spotifyTrack => {
                const isrc = spotifyTrack.external_ids?.isrc ?? "";
                const identifier = spotifyTrack.id ?? "";
                const author = (spotifyTrack.artists ? spotifyTrack.artists[0].name : undefined) ?? "";
                const title = spotifyTrack.name ?? "";
                const uri = spotifyTrack.external_urls?.spotify ?? "";
                const length = spotifyTrack.duration_ms ?? 0;
                const artworkUrl = (spotifyTrack.album?.images[0] ? spotifyTrack.album.images[0].url : undefined) ?? "";
